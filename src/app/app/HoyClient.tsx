...
// ── Hoy — pantalla de apertura del Dashboard (BREY v1.1 + backend) ──────────
...
// que uso hoy". nombre sí es real (viene de page.tsx, Server Component,
// vía Supabase) — el resto de esta pantalla sigue en datos simulados a
// propósito, exactamente como se pidió.
...
export default function HoyClient({ nombre }: { nombre: string }) {
  const [ejercicios, setEjercicios] = useState(entrenamientoHoy.ejercicios);

  const toggle = (nombreEjercicio: string) => {
    setEjercicios((prev) =>
      prev.map((e) => (e.nombre === nombreEjercicio ? { ...e, completado: !e.completado } : e))
    );
  };
